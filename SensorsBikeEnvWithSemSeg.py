...
class BikeEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.sensor_data = {}
        self.bike, self.depth_camera, self.sem_camera, self.collision_sensor = self.spawn_bike()
    ...
    def reset(self):
        ...
        while self.front_camera_depth is None or self.front_camera_semseg is None: 
...
# ================== Hilfsmethoden ==================
    ...
    def spawn_bike(self):
        ...
        sem_camera_bp = self.bp_lib.find('sensor.camera.semantic_segmentation') 
        ...
        sem_camera = self.world.spawn_actor(sem_camera_bp, camera_init_trans, attach_to=bike)
        ...
        image_w_sem = sem_camera_bp.get_attribute("image_size_x").as_int()
        image_h_sem = sem_camera_bp.get_attribute("image_size_y").as_int()
        self.sensor_data = {"depth_image": np.zeros((image_h_depth, image_w_depth, 4)),
                            "sem_image" : np.zeros((image_h_sem, image_w_sem, 4)),
                            "collision": False}
        
        depth_camera.listen(lambda image: self.depth_callback(image, self.sensor_data))
        sem_camera.listen(lambda image: self.sem_callback(image, self.sensor_data))
        ...
        return bike, depth_camera, sem_camera, collision_sensor
    ...
    def sem_callback(self, image, data_dict):
        image.convert(carla.ColorConverter.CityScapesPalette)
        data_dict["sem_image"] = np.reshape(np.copy(image.raw_data), (image.height, image.width, 4))
        self.front_camera_semseg = data_dict["sem_image"]
