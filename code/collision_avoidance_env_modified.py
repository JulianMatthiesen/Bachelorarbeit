...
class BikeEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # synchronous mode und fixed time-step 
...
# ================== Helper Methods ==================
       ...
       # modify for different training structure
        ...
        while True:
            ...
            if spawn_places[0] != spawn_places[1]:
                break
        location1 = carla.Location(x= float(xSpawn[0]), y= float(ySpawn[0]), z=0.05)       # spawn_point_bike
        location2 = carla.Location(x= float(xSpawn[1]), y= float(ySpawn[1]), z=0.0)        # spawn_point_target
    ...
    def calculate_reward(self, action):
        ...
        speed_reward = -0.05 if action[0] > 0.8 or action[0] < -0.8 else 0.05

        reward = speed_reward  
   
        # end episode, when reaching the target, colliding, reaching time limit or leaving the square 
        if self.get_distance_to_target() < 1.0:
            print("\ntarget reached!")
            self.done = True
            reward = 300
